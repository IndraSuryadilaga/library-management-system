...
<div class="max-w-7xl mx-auto">
    <div class="flex justify-between items-center mb-bento-gap">
        <div>
            <h1 class="font-display text-3xl font-semibold text-bark-500">Daftar Denda</h1>
            <p class="font-body text-base text-dusty mt-1">Kelola semua data denda di perpustakaan.</p>
        </div>
        <x-atoms.button variant="primary" href="{{ route('admin.fines.create') }}">
            + Tambah Denda
        </x-atoms.button>
    </div>
    ...
    <x-molecules.table :headers="['ID', 'Pengguna', 'Peminjaman', 'Jumlah', 'Alasan', 'Dibayar Pada', 'Aksi']">
        @forelse ($fines as $fine)
            <tr class="border-b border-cream-200 hover:bg-cream-100/70 transition-colors duration-150">
                ...
                <td class="px-6 py-4 font-body text-dusty">{{ $fine->paid_at ?? '-' }}</td>
                <td class="px-6 py-4 text-center">
                    <div class="flex items-center justify-center space-x-3">
                        <x-atoms.button variant="tertiary" href="{{ route('admin.fines.show', $fine) }}">Lihat</x-atoms.button>
                        <x-atoms.button variant="tertiary" href="{{ route('admin.fines.edit', $fine) }}" class="text-terra-400 hover:text-terra-500">Ubah</x-atoms.button>
            ...
            <tr>
                <td colspan="7" class="text-center py-12">
                    <div class="flex flex-col items-center">
                        ...
                        <p class="font-body text-dusty">Tidak ada data denda.</p>
                        <p class="font-body text-sm text-dusty">Coba tambahkan denda baru.</p>
                    </div>
                </td>
            </tr>
        @endforelse
